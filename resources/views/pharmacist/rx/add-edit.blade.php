...
@section('page-heading', ($_GET['order']) ? (empty($rx) ? 'Create RX' : 'Edit RX') : 'Create Patient')
@section('css')
@stop
@section('content')
@if($_GET['order']=='1')
...
<div id="render-data">
    @if((isset($rx)))
    <form action="{{ route('pharmacist.rx.update',$rx->id) }}" method="POST" class="form">
        @method('PUT')
        @else
        <form action="{{ route('pharmacist.rx.store') }}" method="POST" id="add_form" class="form">
            @endif
            @csrf
            ...
            <div class="card p-3 card-rx-form mb-3">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6">
                        <div class="form-group">
                            <label>DOB *</label>
                            <input name="dob" data-date-format='dd-mm-yyyy' value="{{$rx->dob ?? ''}}" class="form-control datepicker pl-2" required="" autocomplete="off">
                        </div>
                    </div>
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="form-group">
                            <label>CHOOSE SURGERY *</label>
                            <select class="form-control select2" name="surgery_id" id="surgery_id">
                ...
                <div class="row" style="">
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6" style="display: none;">
                        <div class="form-group">
                            ...
                            <input name="order_date" data-date-format='dd-mm-yyyy' class="form-control datepickr pl-2" required="" autocomplete="off">
                        </div>
                    </div>
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="form-group">
                            <label>DATE PRESCRIPTION NEEDED BY * </label>
                            <input name="date_needed" data-date-format='dd-mm-yyyy' class="form-control datepickr pl-2" required="" autocomplete="off">
                        </div>
                    </div>
                    @endif
                    <div class="col-xs-12 col-sm-12 col-md-6" style="display: none;">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="send_via" id="send_via">
                                <option value="">Choose Ordering Method</option>
                                <option value="email">Email</option>
                                <option value="fax">Fax</option>
                                <option value="phone">Phone</option>
                                <option value="other">Other (please state)</option>
                            </select>
                            <input type="text" name="other_ordering_method" value="" class="form-control mt-3" placeholder="Enter Method" style="display: none;">
        ...
        <script>
            ...
            $(".open_modal_new").click(function() {
                ...
                if (test == "email") {
                    $('input[name="other_ordering_method"]').hide();
                    var MOVE = "{{route('pharmacist.loadOrder_new')}}?email=true"
                } else if (test == "fax") {
                    $('input[name="other_ordering_method"]').hide();
                    var MOVE = "{{route('pharmacist.loadOrder_new')}}?fax=true"
                } else if (test == "phone") {
                    $('input[name="other_ordering_method"]').hide();
                    var MOVE = "{{route('pharmacist.loadOrder_new')}}?phone=true"
                } else if (test == "other") {
                    $('input[name="other_ordering_method"]').show();
                    var MOVE = "{{route('pharmacist.loadOrder_new')}}?other=true"
                } else {
                    $('input[name="other_ordering_method"]').hide();
                }
                $.ajax({
        ...
        var test = $('#send_via_render option:selected').val();
        if (test == "email") {
            var MOVE = "{{route('pharmacist.loadOrder')}}?email=true"
        } else if (test == "fax") {
            var MOVE = "{{route('pharmacist.loadOrder')}}?fax=true"
        } else if (test == "phone") {
            var MOVE = "{{route('pharmacist.loadOrder')}}?phone=true"
        } else if (test == "other") {
            var MOVE = "{{route('pharmacist.loadOrder')}}?other=true"
        } else {
            $('input[name="other_ordering_method"]').hide();
        }
        $.ajax({
