                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="action-btn-area">
                            <div class="row-title">{{ $patient->patient ?? "N/A"}}</div>
                            <div class="action-btn">
                                <a href="{{ route('pharmacist.rx.create') }}?value={{$patient->patient}}&rx_id={{$patient->rx_id}}&order=1&today=1" class="text-info">Order</a> |
                                <a data-url="{{route('pharmacist.patient.edit',$patient->id)}}" class="edit_button" href="javascript:;">Edit</a> |
                                <button type="button" onclick="deleteAlert('{{ route('pharmacist.patient.destroy',$patient->id) }}')" title="Delete" class="text-danger">Remove</button>
                            </div>
                        </td>
                        <td>{{$patient->rx->dob ?? "N/A"}}</td>
                        <td>{{ $patient->weeks ?? "N/A"}}</td>
                        <td>
