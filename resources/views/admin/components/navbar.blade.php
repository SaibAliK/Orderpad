<nav class="navbar navbar-top navbar-expand navbar-dark bg-primary border-bottom
  {{ (empty($_GET['order'])) ? 'bg-primary' : 'bg-danger'}} ">
  <div class="container-fluid">
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      ...
      <ul class="navbar-nav align-items-center  ml-auto ml-md-0 ">
        <select name="" id="" class="form-control" onchange="document.location=this.options[this.selectedIndex].value">
          <option hidden>Branch Login</option>
          @foreach(branches() as $pharmacy)
          <option value="{{ route('branch.login', ['id' => $pharmacy->id]) }}">{{ $pharmacy->name }}</option>
          @endforeach
        </select>
