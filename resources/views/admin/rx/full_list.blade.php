...
<div class="text-right mb-3">
    <a class="btn btn-primary btn-sm" href="{{ route('rx.create') }}?order={{0}}"> <i class="fa fa-plus"></i> Create New patient</a>
</div>
            ...
            <tr>
                ...
                <td class="action-btn-area action-btn-area-rx">
                    ...
                    <div class="action-btn">
                        <a href="{{ route('patientEdit') }}?value={{$rx->name}}&rx_id={{$rx->id}}&order={{0}}" class="edit-rx">Edit</a> |
                        <button type="button" onclick="deleteAlert('{{ route('rx.delete',$rx->id) }}')" title="Delete" class="text-danger">Delete</button>
                    </div>
                </td>
                <td>{{ $rx->dob ?? ""}}</td>
