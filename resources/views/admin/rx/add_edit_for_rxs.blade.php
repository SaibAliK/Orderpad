...
<div class="card p-3 card-rx-form mb-3">
    ...
    <div class="col-xs-12 col-sm-12 col-md-12" id="">
        <div class="form-group">
            <label for="">Choose Patient Name *</label>
...
<div id="render-data">
    <form action="{{route('create.uncollected.rxes')}}" method="POST" id="add_form">
        @csrf
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 table-responsive">
                    <table class="table table-bordered tbll">
                        ...
                        <tr class="cloneme">
                            <td>
                                <div class="form-group">
                                    <input type="text" class="form-control" name="unknown_drug[]" placeholder="UnKnown Med" autocomplete="off" required="">
                            ...
                            <td>
                                <input type="text" name="unknown_quantity[]" class="form-control" placeholder="?" autocomplete="off" required="">
                            </td>
                            @if(!empty($_GET['order']) && !empty($rx))
...
<script>
    ...
    $(document).on('click', '.addjob_for_rxes', function() {
        ...
        var $rx_meds = '<tr class="cloneme"><td><input type="text" class="form-control" autocomplete="off" name="unknown_drug[]" placeholder="UnKnown Med"></td><td><input type="text" name="unknown_quantity[]" autocomplete="off" class="form-control" placeholder="?"></td>@if(!empty($_GET['order']) && !empty($rx))<td class="text-center"><a class="rmv btn btn-danger btn-sm text-white mt-2">X</a><button type="button" class="btn btn-sm mt-2"><input type="checkbox" name="is_checked[]" value="1" class="med-st"><input type="hidden" name="checked_hidden[]" value="0" class="med-st"></button></td>@else<td class="text-center"><a class="rmv btn btn-danger btn-sm text-white mt-2">X</a></td>@endif</tr>';
...
<script>
    ...
    $(document).on('click', '.btn-load-patient', function() {
        ...
        if (str != "") {
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('load.rx.patient') }}?value={{$_GET['rx_id'] ?? ''}}&today={{$_GET['today'] ?? ''}}&order={{ $_GET['order'] ?? '' }}&approve={{ $_GET['approve'] ?? '' }}&patient_id={{ $_GET['patient_id'] ?? '' }}&overdue={{$_GET['overdue'] ?? ''}}",
                success: function(response) {
                    if (response.error) {
