        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="action-btn-area">
                    <div class="row-title row-heading-width @if($rx->status == 1) text-danger @endif @if ( \Carbon\Carbon::today()->format('yy-m-d')  == $rx->date_needed_by) text-warning @endif">{{ $rx->name ?? "N/A"}}</div>
                    <div class="action-btn">
                        <div class="display-block">
                            <a href="{{ route('rx.show',$rx->id) }}" class="view-rx">View</a> |
                            @if($rx->patients)
                            <button class="font-weight-bold partially-collect-btn" data-urls="{{ route('rx.medication.all.collect',['id' => $rx->id]) }}?order_id={{$rx->patients->id ?? ''}}" data-id="{{ $rx->id ?? ''}}" data-name="{{$rx->name ?? ''}}" data-dob="{{$rx->dob ?? ''}}"  data-order="{{$rx->patients->id ?? ''}}" data-mds_str_date="{{ date('d-m-Y',strtotime($rx->patients->previous_medication_date)) ?? ''}}" data-span="{{$rx->patients->weeks ?? ''}}">Collect</button>
                            @else
                            @endif
                        </div>
                        {{--@if($rx->status == 1)
                            ...
                            @else
                            <input type="checkbox" class="urgent" name="urgent" value="{{$rx->id}}" data-status="notUrgent" data-toggle="toggle" data-size="xs" data-onstyle="success" {{ $rx->status == 1 ? 'checked' : '' }}> |
                            @endif--}}
                            <button type="button" onclick="removeAlert('{{ route('rx.destroy',$rx->id) }}')" title="Delete" class="text-danger">Remove</button>
                        </div>
                    </td>
                    <td>{{ $rx->dob ?? "N/A"}}</td>
    ...
    <div class="modal" tabindex="-1" role="dialog" id="partially_collect_modal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('rx.popup.medication.collection.update') }}" method="post" class="form">
    ...
    <div class="modal" tabindex="-1" role="dialog" id="second_modal" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('rx.new.popup')}}?popup=second" method="post" class="second_form">
                    <div class="modal-body pt-0 pb-0">
                        @csrf
                        <div class="row">
    ...
    <div class="modal" tabindex="-1" role="dialog" id="third_modal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('rx.new.popup')}}?popup=third" method="post" class="third_form">
                    <div class="modal-body pt-0 pb-0">
                        @csrf
                        <div id="" class="row">
                            <input type="hidden" name="rx_id" value=""  class="rx_id">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-secondary close_modal" type="button" data-closeType="third">Go Back</button>
                        <button type="submit" class="btn btn-primary">Proceed</button>
                    </div>
                </form>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click','.close_modal',function(){
                ...
                {
                    var MOVE = "{{ route('rx.popup.medication.collection.update') }}?close_type=" + close_type;
                    $.ajax({
                        type: "POST",
                        url: MOVE,
            ...
            $(document).on('change', '.urgent', function() {
                var abc = $(this).val();
                if ($(this).data('status') == "urgent") {
                    var ur = "{{route('urgentRX')}}?id=" + abc + "&value=urgent";
                } else {
                    var ur = "{{route('urgentRX')}}?id=" + abc + "&value=notUrgent";
                }
                $.ajax({
                    type: "GET",
                    url: ur,
            ...
            $(document).on('click', '.partially-collect-btn', function() {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ route('rx.popup.medication.collection') }}?id=" + id + "&order_id=" + order_id,
                    success: function(response) {
                        console.log(response);
                        if (response.error) {
