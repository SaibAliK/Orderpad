                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="action-btn-area">
                            <div class="row-title">{{ $order->patient ?? "N/A"}}</div>
                            <div class="action-btn">
                                @if($order->previous_order_date !== $date)
                                <a href="{{ route('rx.create') }}?value={{$order->patient}}&rx_id={{$order->rx_id}}&order=1&today=1&approve=true&patient_id={{$order->id}}" class="text-info">Order</a> |
                                @else
                                <a href="{{ route('rx.create') }}?value={{$order->patient}}&rx_id={{$order->rx_id}}&order=1&today=1&approve=false" class="text-info">Order</a> |
                                @endif
                                <a data-url="{{route('patient.edit',$order->id)}}" class="edit_button" href="javascript:;">Edit</a> |
                                <button type="button" onclick="deleteAlert('{{ route('patient.destroy',$order->id) }}')" title="Dwelete" class="text-danger">Remove</button>
                            </div>
                        </td>
                        ...
                        <td>
                            {{ $order->previous_order_date == $date ? "" : "To Be Ordered"}}
                        </td>
                    </tr>
                    @endif
                    @endforeach
