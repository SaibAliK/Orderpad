...
<div class="row">
    ...
    <div class="col-xs-12 col-sm-12 col-md-6" style="display: none;">
        <div class="form-group">
            ...
            <select class="form-control" name="pharmacy_id">
                <option hidden selected="" value="">{{ $rx->user->name ?? 'Choose Pharmacy' }}</option>
                @foreach(pharmacies() as $pharmacy)
                <option value="{{ $pharmacy->id }}" {{ $pharmacy->name == $rx->user->name ? 'selected' : '' }}>{{ $pharmacy->name ?? ''}}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div class="col-xs-12 col-sm-12 col-md-6" style="display: none;">
        <div class="form-group">
            <strong>DOB *</strong>
            <input type="text" name="dob" placeholder="DOB" data-date-format='dd-mm-yyyy' value="" class="form-control datepicker pl-2" required="" readonly="">
        </div>
    </div>
