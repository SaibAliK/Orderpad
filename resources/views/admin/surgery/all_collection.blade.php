        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="action-btn-area">
                    <div class="row-title @if($surgery->rxes()->where('status', 1)->exists()) text-danger @elseif( $surgery->rxes()->where('date_needed_by',\Carbon\Carbon::today()->format('yy-m-d'))->exists()) text-warning @endif">{{ $surgery->name }}</div>
                    <div class="action-btn">
                        <a href="{{ route('rx.collection') }}?surgery={{ $surgery->id }}&show_surgery=true">View Rxs</a> |
