  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        {{--<h5 class="modal-title">Order Detail</h5>uncollected --}}
          ...
          <div class="col-md-12 mt-3" style="line-height: 0.7">
            ...
            @else
              ...
              <p>Order by: {{ $rx->order_by ?? "N/A"}}</p>
                ...
                @if(empty($rx->collected_at))
                <p>Order sent on: {{$rx->patients->order_date ?? 'N/A'}}</p>
                <p>Medication start date: {{$rx->patients->medication_date ?? 'N/A'}}</p>
                @else
                ...
                Marks as Urgent:
                @if($rx->status == 1)
                  <input type="checkbox" class="urgent btn btn-lg" name="urgent" value="{{$rx->id}}" data-status="urgent" data-toggle="toggle" data-size="xs" data-onstyle="success" {{ $rx->status == 1 ? 'checked' : '' }}>
                  @else
                  <input type="checkbox" class="urgent btn btn-lg" name="urgent" value="{{$rx->id}}" data-status="notUrgent" data-toggle="toggle" data-size="xs" data-onstyle="success" {{ $rx->status == 1 ? 'checked' : '' }}>
                  @endif
            @endif
          </div>
        </div>
