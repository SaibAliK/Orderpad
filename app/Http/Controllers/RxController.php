...
class RxController extends Controller
{
    ...
    {
        if ($req->pharmacy) {
            $rxes = Rx::where('pharmacy_id', $req->pharmacy)->get();
            $pharmacy = User::find($req->pharmacy);
        } else
            $rxes = Rx::where('status', 1)->get();
        return view('admin.rx.rx', get_defined_vars());
    }
    public function create(Request $req)
    {
        ...
        if ($req->rxes_B == "true") {
            ...
            return view('admin.rx.add_edit_for_rxs', get_defined_vars());
        }
        if ($req->rx_id) {
            $rxes = Rx::withTrashed()->get();
        } else {
        ...
        $drugs = Drug::all();
        return view('admin.rx.add-edit', get_defined_vars());
    }
    public function rxsSave(Request $request)
    {
        request()->validate([
            'name' => 'required',
            'surgery_id' => 'required',
        ...
        if ($request->rx_id) {
            ...
            $rx = Rx::where('name', $request->name)->first();
        ...
        if ($request->orderpad == 'false') {
            ...
            if (empty($request->rx_id))
                $rx->delete();
            if ($request->orderpad == 'false') {
                return redirect()->route('rx.full_list')->with('success_msg', 'New Patient has been created.');
            }
            return redirect()->route('rx.full_list')->with('success_msg', 'New Rx has been created.');
        } else {
            $rx->name = $request->name;
            $rx->dob = $request->dob;
            $rx->surgery_id = $request->surgery_id;
        ...
        if (empty($request->orderpad)) {
            return redirect()->route('rx.full_list')->with('success_msg', 'Patient update successfully');
        }
        return redirect()->route('rx.collection')->with('success_msg', 'New RX has been created.');
    }
    ...
    public function saveUncollectedRx(Request $request)
    {
        ...
        $rx = RX::withTrashed()->where('name', $request->name)->first();
        if ($rx) {
            return redirect()->back()->with('error_msg', 'Error RX already exists.');
        }
        $rx = new Rx();
        $rx->name = $request->name;
        $rx->dob = $request->dob;
    ...
    public function urgentRX(Request $request)
    {
        if ($request->id) {
    ...
    public function patientEdit(Request $req)
    {
        ...
        if ($req->rx_id) {
            ...
            return view('admin.rx.add-edit-patient', get_defined_vars());
        }
    }
    ...
    public function destroy($id)
    {
        $rx = Rx::find($id);
        $rx->delete();
        return redirect()->route('rx.collection')->with('success_msg', 'List has been updated.');
    }
    ...
    public function rxCollection(Request $req)
    {
        ...
        $rxes = Rx::with('patients')->whereHas('medications', function ($query) {
        ...
        if ($req->show_surgery == "true") {
            return view('admin.surgery.collection', get_defined_vars());
        }
        return view('admin.rx.collection', get_defined_vars());
    }
    public function rxCollected()
    {
        $rxes = Rx::get();
        return view('admin.rx.collected', get_defined_vars());
    }
    ...
    //partially collected button
    public function rxPopupMedicationCollectionUpdate(Request $request)
    {
        //dd($request->all());
        //this also handle the close popup functionality
    ...
    public function loadRxPatient(Request $request)
    {
        ...
        $today = $request->today;
        if (empty($request->patient_id) && !empty($request->value) && empty($request->today)) {
            $rx = Rx::withTrashed()->where('id', $request->value)->first();
            if (is_null($rx)) {
                return response()->json(['error' => '1'], 200);
            } else {
                $drugs = Drug::all();
                $html = view('admin.rx.add-edit-render_patient', get_defined_vars())->render();
                return response()->json(['html' => $html], 200);
            }
        } else {
            $rx = Rx::withTrashed()->where('id', $request->value)->first();
            if (is_null($rx)) {
                ...
                $drugs = Drug::all();
                $html = view('admin.rx.add-edit-render', get_defined_vars())->render();
                return response()->json(['html' => $html], 200);
            }
        }
    }
    ...
    {
        $rxes = Rx::withTrashed()->get();
        /*$rxes = Rx::withTrashed()->whereHas('medications', function ($query) {
        $query->where('status', '0');
    })->get();*/
        $with_trashed = 1;
        return view('admin.rx.full_list', get_defined_vars());
    }
    public function faxes()
    {
        $current_date = Carbon::now();
        $rxes = Rx::where('send_via', 'fax')->where('today', '1')->get();
        return view('admin.rx.faxes', get_defined_vars());
    }
    public function showFax($id)
    {
        $rx = Rx::withTrashed()->find($id);
        return view('admin.rx.faxShow', get_defined_vars());
    }
    ...
    public function loadOrder(Request $request)
    {
        ...
        if (isset($request->email)) {
            $rx = Rx::withTrashed()->find($request->rx_id);
            if ($rx) {
                return view('admin.rx.load_email_order', get_defined_vars());
            }
        } else {
            $rx = Rx::withTrashed()->find($request->rx_id);
            return view('admin.rx.loadOrder', get_defined_vars());
        }
    }

    public function loadOrder_new(Request $request)
    {
        if ($request->surgery_id) {
            if ($request->email) {
                $surgery = Surgery::find($request->surgery_id);
                ...
                $surgery = Surgery::find($request->surgery_id);
                return view('render.loadOrder', get_defined_vars());
            }
        } else {
    ...
    function autoDrugs(Request $req)
    {
        //dd($req->all());
        $arr = array();
        $drugs = DB::table('drugs')->where('name', 'like', '%' . $req->q . '%')->get();
